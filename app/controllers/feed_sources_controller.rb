class FeedSourcesController < ApplicationController
  before_action :set_feed_source, only: [:show, :edit, :update, :destroy]
  ...
  def create
    ...
    respond_to do |format|
      if @feed_source.save
        ...
        format.json { render action: 'show', status: :created, location: @feed_source }
      else
  ...
  def destroy
    @feed_source.destroy
    respond_to do |format|
      format.html { redirect_to feed_sources_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def feed_source_params
      params.require(:feed_source).permit(:name, :url)
    end
end
